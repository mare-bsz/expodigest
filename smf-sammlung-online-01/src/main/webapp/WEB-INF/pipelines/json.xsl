		...
		{		
    		...
    		<xsl:template name="ueberschriften">
    			<xsl:choose>
	    			...
	    			<xsl:otherwise>
	    				<xsl:text>, "ueberschrift1de" : "</xsl:text><xsl:value-of select="java:bsz.expo.Util.toJson(./objektbezeichnungen/./deskriptor[1]/term)"/><xsl:text>"</xsl:text>
        	...
        	<xsl:template match="objektbezeichnungen">
    			<xsl:text>,"objektbezeichnungde" : "</xsl:text><xsl:value-of select="./deskriptor[1]/term"/><xsl:text>"</xsl:text>
    		</xsl:template>
        	...
        	<xsl:template match="entstehungszeit">
    			<xsl:text>,"entstehungszeit" : "</xsl:text><xsl:value-of select="java:bsz.expo.Util.toJson(.)"/><xsl:text>"</xsl:text>
    		</xsl:template>
